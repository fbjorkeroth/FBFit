BeginPackage["FBFit`MixingParameterTools`MPT3x3`"]
...
(* Takagi diagonalization: Implementation by Vinzenz Maurer following the algorithm described in arXiv:physics/0607103 [physics.comp-ph] *)

ClearAll[Diag,Phase,UTakagi,UTakagiCompiled,TakagiDecompositionStep];
ClearAll[MPTEigensystem,MPTEigensystemNew,MPTEigensystemOld];
...
TakagiDecomposition[M_] := Block[{u, d, \[CurlyPhi]},
  {u, d} = FixedPoint[
...
MPTEigensystemNew[M_] := Block[{u,d,tmpU,scale,MM},
(*
returns a list of pairs of eigenvalue and properly normalised eigenvector
the list is ordered from the smallest to the largest eigenvalue 
*)
			       scale=Sqrt[Max[Abs[M]]*Min[Select[Flatten[Abs[M]], # > 0 &]]];
			       MM=M/scale;
			       
			    MPTDebugPrint[M];
{u,d}=TakagiDecomposition[MM];
			       d=d*scale;
			    MPTDebugPrint[{d,Chop[u.M.Transpose[u]]}];
			    tmpU=MapThread[{#1,#2}&,{Diagonal[d],u}];
Return[Sort[tmpU,#1[[1]]<#2[[1]]&]];
];


MPTEigensystemOld[M_]:=Block[{evalues,evectors,iLauf,iLen,tmpVec,tmpU},
(* old version of MPTEigensystem extracted from code *)
			     iLen=Length[M];
tmpU = {}; 
(*  diagonalize mixing matrix *)
{evalues, evectors} = Eigensystem[N[Conjugate[Transpose[M]] . M]]; 
MPTDebugPrint[Chop[Conjugate[Transpose[evectors]] . evectors]];
MPTDebugPrint[Eigensystem[N[Conjugate[Transpose[M]] . M]]];

(* square root of eigenvalues *) 
evalues = Sqrt[evalues];	 

(* normalisation of eigenvectors and something ?? *)
...
(* orthogonalise *)
If[iLauf < iLen, tmpVec = evectors[[iLauf]] - Sum[Conjugate[evectors[[jLauf]]] . evectors[[iLauf]]* evectors[[jLauf]], {jLauf, 1, iLauf - 1}]]; 
...
(* sort eigenvectors according to eigenvalue *)
tmpU = Sort[tmpU, Abs[#1[[1]]] < Abs[#2[[1]]] & ]; 
Return[tmpU];
];

MPTEigensystem[M_]:=MPTEigensystemNew[M];
(* end Takagi diagonalization *)
...
(* use Takagi diagonalisation *)
tmpU = MPTEigensystem[N[M]];
evectors=Map[#[[2]]&,tmpU];
(* end *)
...
MPT3x3NeutrinoMixingMatrix[M_?MPTNumericMatrixQ, S_,pOpts___Rule] := 
	Block[{tmpU, tmpVec, evalues, evectors, iLauf, jLauf, iLen, tmpV, 
	 ...
	 If[MatrixNorm[M - Transpose[M]] > 0.01, 
       Throw["MPT3x3NeutrinoMixingMatrix::Encountered non-symmetric matrix!"]]; iLen = Length[M[[1]]];

(* use Takagi diagonalisation *)
tmpU=MPTEigensystem[N[M]];
evectors=Map[#[[2]]&,tmpU];
(* finished using Takagi diagonalisation *)
